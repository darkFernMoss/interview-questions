# 设计模式
[23 种设计模式详解（全23种）_鬼灭之刃的博客-CSDN博客](https://blog.csdn.net/A1342772/article/details/91349142)

## 前置知识
#### 什么是7大原则
开放封闭原则的核心思想可以概括为：

**软件实体（类、模块、函数等）应该对扩展是开放的，对修改是封闭的。**

![[Pasted image 20230817234348.png]]
...
简单工厂模式有一个问题就是，类的创建依赖工厂类，**如果想要拓展程序，必须对工厂类进行修改，这违背了开闭原则**，所以，从设计角度考虑，有一定的问题，如何解决？我们可以定义一个创建对象的抽象方法并创建多个不同的工厂类实现该抽象方法，这样一旦需要增加新的功能，直接增加新的工厂类就可以了，不需要修改之前的代码。这种方法也就是我们接下来要说的工厂方法模式。
...
![[Pasted image 20230817234915.png]]
...
3. 抽象工厂 ：用来生产不同产品族的全部产品。（支持拓展增加产品；支持增加产品族）
...
![[Pasted image 20230817235544.png]]
...
![[Pasted image 20230817235852.png]]
#### 3.2 生成器模式的优缺点
优点
将一个对象分解为各个组件
...
#### 4.2 原型模式的结构和代码示例
![[Pasted image 20230818000133.png]]
...
![[Pasted image 20230818000709.png]]
...
举例(将USB接口转为VGA接口)，类图如下：
 ...
 ![[Pasted image 20230818001406.png]]
...
![[Pasted image 20230818001657.png]]
...
![[Pasted image 20230818002023.png]]
...
![[Pasted image 20230818013059.png]]
...
举例(买房），类图如下：![[Pasted image 20230818013432.png]]
```java
第一步：创建服务类接口
...
#### 8.1 模式结构和代码示例
![[Pasted image 20230818014553.png]]
...
![[Pasted image 20230818014747.png]]
...
换一种解决思路![[Pasted image 20230818014816.png]]
...
![[Pasted image 20230818014854.png]]

#### 9.2 桥接模式结构和代码示例

![[Pasted image 20230818014904.png]]
```java
...
不容易用继承的方法来增加构件的新功能；
#### 10.1 模式结构和代码示例
![[Pasted image 20230818015745.png]]
...
#### 11.1 享元模式的结构图和代码示例
![[Pasted image 20230818020313.png]]
...
举例（JAVA 中的 String，如果有则返回，如果没有则创建一个字符串保存在字符串缓存池里面）。类图如下：![[Pasted image 20230818020326.png]]
...
第四类：通过中间类
![[Pasted image 20230818020455.png]]
### 12 策略模式
...
#### 12.1 策略模式结构和示例代码
![[Pasted image 20230818020641.png]]
...
![[Pasted image 20230818020712.png]]
...
#### 13.1 模式结构和代码示例
![[Pasted image 20230818020844.png]]

![[Pasted image 20230818020957.png]]
```java
a. 先来写一个抽象的做菜父类： 
...
#### 14.1 模式结构图和代码示例
![[Pasted image 20230818021127.png]]
...
![[Pasted image 20230818021254.png]]
```java
1、定义一个抽象被观察者接口
...
缺点：由于迭代器模式将存储数据和遍历数据的职责分离，增加新的聚合类需要对应增加新的迭代器类，类的个数**成对增加**，这在一定程度上增加了系统的复杂性。

 #### 15.1 模式结构和代码示例
 ![[Pasted image 20230818021608.png]]
...
![[Pasted image 20230818021850.png]]
...
![[Pasted image 20230818021923.png]]
```java
...
#### 17.1模式结构和代码示例
![[Pasted image 20230818022728.png]]
...
代码举例（开灯和关灯），类图如下：
![[Pasted image 20230818022908.png]]
...
#### 18.1 模式结构和代码示例
![[Pasted image 20230818023504.png]]
1. State抽象状态角色

2. 接口或抽象类，负责对象状态定义，并且封装环境角色以实现状态切换。

3. ConcreteState具体状态角色

4. 具体状态主要有两个职责：一是处理本状态下的事情，二是从本状态如何过渡到其他状态。

5. Context环境角色

6. 定义客户端需要的接口，并且负责具体状态的切换。
...
#### 19.1 模式结构图和代码示例
![[Pasted image 20230818023939.png]]
...
![[Pasted image 20230818024219.png]]
```java
1 备忘录接口
...
#### 20.1 模式结构和代码示例
![[Pasted image 20230818024508.png]]
访问者模式包含以下主要角色。
...
#### 21.1 模式结构和代码示例
![[Pasted image 20230818024855.png]]
...
举例（通过中介卖方），类图如下：![[Pasted image 20230818024946.png]]
